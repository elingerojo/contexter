...
// ... if ALL plugins have it, then a glob be built to optimize watcher
...
module.exports = class Contexter {
  ...
  watcher (sourceDir, options) {
    ...
    // Form a watch glob like `<source-dir>/**/*.{ext1,ext2,...,extn}`
    function optimizeGlob(sourceDir) {
      ...
      // ... `watchExtensions` in all plugins
      if (isStillOptimizable) {
        watchGlob = sourceDir + '/**/*.{'

...
const defautlPlugins  = require('require-dir')('./plugins')
const defaultTypes    = require('require-dir')('./filetypes')
...
module.exports = class Contexter {
  ...
  // Add custom file types by creating or extending a particular ...
  // ... `this.filetypes` (Ex. creating `image` file type or extending ...
  // ... existing `datafile`)
  // extend API:
  // `.extend(string-name-of-filetype, object-filetype-with-custom-overrides)`
  ...
  // Adds custom plugins to the plugins list
  // A plugin is an object with properties and hi-level functions like ...
  // ... `parse()` and `render()` to process a particular file type
  use (plugin) {
    // TODO: Add some validation

    this.plugins.push(plugin)

    // Accumulate all possible watch file extensions and the ...
    // ... `watchExtensions` property existance in plugins to determine ...
    // ... watch optimization capability
  ...
  watcher (sourceDir, options) {
    ...
    function createFile(filename) {
      ...
      // ... used for write-like functions like `render()`
      let targetExt = typeof checkResult === 'string'
        ? checkResult
        : path.extname(filename).toLowerCase()

      // Add to plugin config (mandatory by plugins with write-like functions)
      Object.assign(self.pluginConfig, {targetExt})

      const file
        = ctx.newFile(filename, sourceDir, selectedPlugin, self.pluginConfig)
      // Run "contextualizing recipe" steps to establish file relations (DO)
      file.recipeDO(ctx)
    ...
    function updateFile(filename) {
      ...
      // Just update content, no need run "contextualizing recipe"
      file.squeeze()
    ...
    function deleteFile(filename) {
      var file = self.files.find(f => f.path.full === filename)
      // Run "contextualizing recipe" steps to remove file relations (UNDO)
      file.recipeUNDO(ctx)
